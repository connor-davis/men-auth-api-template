...
router.post("/", async (request, response) => {
    ...
    try {
        ...
        if (userFound) return response.status(500).json({ message: "Email already in use. Please use a different email.", error: "E00009" })
        else {
        ...
        return response.status(500).json({ message: "Failed to edit user data.", reason: error, error: "E00008" });
    }
});
